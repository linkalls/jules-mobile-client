...
- 📸 **写真アップロード準備完了** - 写真添付のUIを実装（API対応待ち）
- 🎯 **モード選択** - 実行（自動実行）または確認（手動承認）モードを選択
- 📋 **プラン承認フロー** - AIが生成したプランを実行前に確認・承認
- 🔄 **セッション状態追跡** - リアルタイムの色分け状態バッジ表示

**最新アップデートの新機能：**
- 🎯 **モード選択** - セッション作成時に実行（自動実行）または確認（手動承認）モードを選択
- 📋 **プラン承認フロー** - 確認モードでAI生成プランを実行前にレビュー
- 🔄 **セッション状態追跡** - リアルタイムの色分け状態バッジ（待機中、計画中、実行中など）
- 🔍 **検索とフィルター** - ステータスでのフィルタリングと検索で素早くセッションを発見
- 📊 **ソートオプション** - 日付（新しい順/古い順）またはタイトルでソート
